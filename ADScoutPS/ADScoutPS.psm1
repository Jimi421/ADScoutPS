...
<#
.SYNOPSIS
Finds non-DC accounts configured for unconstrained delegation.
#>
    [CmdletBinding()]
    ...
    $items | Where-Object { $_.UacFlags -match 'TRUSTED_FOR_DELEGATION' -and ($IncludeDomainControllers -or $_.PrimaryGroupId -ne '516') }
}
...
#>
    ...
    [PSCustomObject]@{ PolicyType='DefaultDomain'; Name='Default Domain Policy'; DistinguishedName=$ctx.SearchBase; MinPwdLength=$entry.Properties['minPwdLength'][0]; PwdHistoryLength=$entry.Properties['pwdHistoryLength'][0]; LockoutThreshold=$entry.Properties['lockoutThreshold'][0]; LockoutDisabled=([int]($entry.Properties['lockoutThreshold'][0]) -eq 0); MaxPwdAgeRaw=$entry.Properties['maxPwdAge'][0]; MinPwdAgeRaw=$entry.Properties['minPwdAge'][0] }
    ...
    foreach ($r in $s.FindAll()) { [PSCustomObject]@{ PolicyType='FineGrained'; Name=Get-ADScoutProperty $r 'name'; DistinguishedName=Get-ADScoutProperty $r 'distinguishedname'; Precedence=Get-ADScoutProperty $r 'msds-passwordsettingsprecedence'; MinPwdLength=Get-ADScoutProperty $r 'msds-minimumpasswordlength'; PwdHistoryLength=Get-ADScoutProperty $r 'msds-passwordhistorylength'; LockoutThreshold=Get-ADScoutProperty $r 'msds-lockoutthreshold'; AppliesTo=Get-ADScoutProperty $r 'msds-psoappliesto' } }
}
...
<#
.SYNOPSIS
Gets direct or recursive group members.
#>
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$Identity,[switch]$Recursive,[string]$Server,[PSCredential]$Credential,[string]$SearchBase)
    $dn = Resolve-ADScoutDistinguishedName -Identity $Identity -ObjectClass group -Server $Server -Credential $Credential -SearchBase $SearchBase
    $s = New-ADScoutSearcher -Filter "(distinguishedName=$dn)" -Properties @('member') -Server $Server -Credential $Credential -SearchBase $SearchBase -SizeLimit 1
    $r = $s.FindOne(); if (-not $r) { return }
    $members = (Get-ADScoutProperty $r 'member') -split ';' | Where-Object { $_ }
    foreach ($m in $members) { [PSCustomObject]@{ Group=$Identity; MemberDistinguishedName=$m; Recursive=$false } }
    if ($Recursive) { foreach ($m in $members) { if ($m -match '^CN=') { try { Get-ADScoutGroupMember -Identity $m -Recursive -Server $Server -Credential $Credential -SearchBase $SearchBase | ForEach-Object { $_.Recursive=$true; $_ } } catch {} } } }
}
...
#>
    [CmdletBinding()]
    param([string]$Server,[PSCredential]$Credential,[string]$SearchBase)
    foreach ($g in 'Domain Admins','Enterprise Admins','Schema Admins','Administrators','Account Operators','Backup Operators','Server Operators') {
        try { Get-ADScoutGroupMember -Identity $g -Recursive -Server $Server -Credential $Credential -SearchBase $SearchBase | ForEach-Object { $_ | Add-Member -NotePropertyName PrivilegedGroup -NotePropertyValue $g -PassThru } } catch {}
    }
}
...
#>
    ...
    Write-Host '[*] ADScoutPS v0.7 collection starting...' -ForegroundColor Cyan
    $data=[ordered]@{}
    ...
    if (-not $SkipAclSweep) { $data.DCSyncRights=@(Find-ADScoutDCSyncRight -Server $Server -Credential $Credential -SearchBase $SearchBase) }
